...
// void	ft_dda_texture(t_all *all) ///// doppia /////
// {
// 	if (all->ray.side == 0 || all->ray.side == 2)
// 		all->ray.perp_wall_dist = ((all->ray.map_x - all->player.pos_x + (1 - all->ray.step_x) / 2) / all->ray.ray_dir_x);
// 	else
// 		all->ray.perp_wall_dist = ((all->ray.map_y - all->player.pos_y + (1 - all->ray.step_y) / 2) / all->ray.ray_dir_y);
// }
...
void	ft_put_texture(int x, t_all *all)
{
	...
	all->texture.wall_x -= floor(all->texture.wall_x); //floor è definito in math.h
	...
	if (all->ray.side == 1 && all->ray.ray_dir_y < 0)
		all->texture.texture_x = all->north.width - all->texture.texture_x - 1;
		//inserire altri lati texture qui
