...
void	find_resolution_support_1(t_data *p, char *line, int *i)
{
...
t_data	*find_resolution(t_data *p, char *line)
{
	...
	p->z_buffer = malloc(sizeof(double) * p->screen_width + 1);
	if (!(p->z_buffer))
...
void	check_error_texture_support(char *to_check, t_data *p)
{
