...
/*   Updated: 2021/04/06 08:58:49 by stribuzi         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
void	screenshot_support_1(t_data *p, char *header)
{
	int		size;
...
int	write_screenshot(t_data *p, unsigned char *pixels)
{
	...
	width_in_bytes = p->screen_width * 3;
	padding_size = (4 - (width_in_bytes) % 4) % 4;
	stride = (width_in_bytes) + padding_size;
	fd = open("./screen.bmp", O_CREAT | O_RDWR, 0666);
	if (fd < 0)
		return (-1);
	write(fd, create_bitmap_file_header(p->screen_height, stride),
			FILE_HEADER_SIZE);
	write(fd, create_bitmap_info_header(p->screen_height, p->screen_width),
			INFO_HEADER_SIZE);
