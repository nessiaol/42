...
/*   Updated: 2021/03/30 18:05:51 by stribuzi         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */

#include "parser.h"

char	*find_texture_support_1(int pos, char *line, char *to_return, t_data *p)
{
	int	tex_len;

	tex_len = 0;
	while (is_printable((int)line[pos]) == 1)
	{
...
char	*find_texture(char *line, char *to_check, t_data *p)
{
	...
	while (is_space(line[i]) == 1)
		i++;
	pos = i;
	while (is_printable((int)line[i++]) == 1)
		tex_len++;
