...
/*   Updated: 2021/03/30 18:02:30 by stribuzi         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */

#include "parser.h"

void	check_found(t_data *p, t_found found)
{
...
void	parse_info_support_1(t_data *p, char *line, int i)
{
	i = 0;
	while (line[i])
	{
...
void	parse_info_support_2(t_data *p, char *line, int *map_height)
{
	if (only_number_and_spaces(line) == 1 && only_spaces(line) == 0)
	{
		count_num_of_sprites_in_line(&p->num_sprite, line);
		map_height[0]++;
	}
}
