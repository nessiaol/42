...
int		ft_keypress(t_all *all)
{
	...
	if (all->keyboard.keyboard[2]) //DESTRA
		ft_move_right(all);
	if (all->keyboard.keyboard[46]) //	MINIMAPPA DA FIXARE 
	{
	...
	if (all->keyboard.keyboard[124]) //ROTAZIONE A DX
		ft_rot_camera_right(all);
	if (all->keyboard.keyboard[123])	//ROTAZIONE VISUALE A SX
		ft_rot_camera_left(all);
